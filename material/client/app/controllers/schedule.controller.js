        ...
        $scope.canSubmit = function() {
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                if (temp.length != 13 || !phone.test(temp[2]) || !time.test(temp[3]) || !time.test(temp[4]) ||
                    ...
                    !time.test(temp[9]) || !time.test(temp[10]) || !time.test(temp[11]) || !time.test(temp[12])) {
                    return false;
                }

                if (courses.hasOwnProperty(temp[0])) {
                    var course = courses[temp[0]]
                } else {
                    return false;
                }
                console.log('here1');
                ...
                if (subjects.hasOwnProperty(course._id + '_' + temp[1])) {
                    var subject = subjects[course._id + '_' + temp[1]]
                } else {
                    return false;
                }
                console.log('here2');
                if (users.hasOwnProperty(temp[2])) {
                    var professor = users[temp[2]]
                } else {
                    return false;
                }
                console.log(subject);
